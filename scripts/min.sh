...
# @@File             :  min.sh
# @@Description      :  Script to setup min for Debian
...
RELEASE_TYPE="$(. /etc/os-release 2>/dev/null; { [[ " $ID_LIKE " == *debian* ]] || [ "$ID" = "debian" ]; } && echo "debian")"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
if ! grep --no-filename -sE -- '^ID=|^ID_LIKE=|^NAME=' /etc/*-release | grep -qiwE -- "debian"; then
...
__retrieve_version_file() { __grab_remote_file "https://github.com/casjay-base/debian/raw/main/version.txt" | head -n1 || echo "Unknown version"; }
__domain_name() {
	local d="" f=""
...
' "${YELLOW}Adding Docker apt repository${NC}"
		...
		_docker_id="$(. /etc/os-release && echo "$ID")"
		_docker_codename="$(. /etc/os-release && echo "$VERSION_CODENAME")"
		echo "deb [arch=$(dpkg --print-architecture) signed-by=/etc/apt/keyrings/docker.gpg] https://download.docker.com/linux/$_docker_id $_docker_codename stable" \
			| tee /etc/apt/sources.list.d/docker.list >/dev/null
		unset _docker_id _docker_codename
	fi
...
# __install_pkg cronie-noanacron  # skipped on debian
...
# __install_pkg biosdevname  # skipped on debian
...
# __install_pkg cockpit-bridge  # skipped on debian
# __install_pkg cockpit-system  # skipped on debian
# __install_pkg cockpit-ws  # skipped on debian
...
# __install_pkg grub2-tools-extra  # skipped on debian
# __install_pkg grubby  # skipped on debian
__install_pkg gzip
...
# __install_pkg mod_geoip  # skipped on debian
# __install_pkg mod_http2  # skipped on debian
# __install_pkg mod_maxminddb  # skipped on debian
__install_pkg libapache2-mod-perl2
# __install_pkg mod_ssl  # skipped on debian
__install_pkg libapache2-mod-wsgi-py3
# __install_pkg mod_proxy_html  # skipped on debian
...
# __install_pkg perl-DBD-Firebird  # skipped on debian
...
__install_pkg postfix
# __install_pkg postfix-pcre  # skipped on debian
__install_pkg python3-certbot-dns-rfc2136
__install_pkg python3-configargparse
__install_pkg python3-cryptography
# __install_pkg python3-enum34  # skipped on debian
# __install_pkg python3-funcsigs  # skipped on debian
__install_pkg python3-future
__install_pkg python3-idna
# __install_pkg python3-josepy  # skipped on debian
# __install_pkg python3-mock  # skipped on debian
__install_pkg python3-pynvim
# __install_pkg python3-parsedatetime  # skipped on debian
# __install_pkg python3-pbr  # skipped on debian
__install_pkg python3-pip
__install_pkg python3-psutil
# __install_pkg python3-pyasn1  # skipped on debian
# __install_pkg python3-pyrfc3339  # skipped on debian
# __install_pkg python3-pysocks  # skipped on debian
__install_pkg python3-requests
# __install_pkg python3-six  # skipped on debian
__install_pkg python3-virtualenv
__install_pkg libreadline-dev
# __install_pkg rootfiles  # skipped on debian
__install_pkg rsync
...
__devnull git clone -q "https://github.com/casjay-base/debian" "$CONFIG_TEMP_DIR"
...
_default_web_assets_script="$(curl -LSs "https://github.com/casjay-templates/default-web-assets/raw/main/setup.sh")"
run_post_message="Installing default server files" __run_post sudo -HE STATICSITE="$(hostname -f)" bash -c "$_default_web_assets_script"
unset _default_web_assets_script
